# ブログ更新・UR空室通知スクリプト

## 概要

このプロジェクトは、以下の2つの情報を定期的にチェックし、更新があった場合にGmail経由で通知を送るPythonスクリプトです。

1. **ブログ更新**: [タカギプランニングオフィスブログ](https://www.t-p-o.com/blog/)の新着記事
2. **UR賃貸空室**: 品川区・港区・目黒区のUR賃貸物件の空室情報

処理はGitHub Actionsによって自動化されており、毎日定刻に実行されます。

## 主な機能

### ブログ更新チェック
- 指定されたWebページから最新のブログ記事情報を取得
- 前回の記事情報と比較し、更新があるかを確認
- 更新があった場合、通知メールを送信

### UR空室チェック
- 品川区・港区・目黒区の指定されたUR賃貸物件の空室状況を監視
- 空室数が増加した場合（0→1、1→2など）に通知メールを送信
- Seleniumを使用して動的に読み込まれる空室情報を取得
- 各物件の空室数を`ur_properties.json`に記録して変化を追跡

### 共通機能
- GitHub Actionsによる定期的な自動実行
- 各チェック結果を個別のメールで通知

## セットアップ方法

この自動通知システムを利用するには、以下の手順でセットアップが必要です。

1.  **リポジトリの準備**
    このリポジトリを自身のGitHubアカウントにフォークまたはクローンします。

2.  **GitHub Secretsの設定**
    本スクリプトは、メール送信のためにGmailの認証情報を必要とします。安全に情報を管理するため、GitHubリポジトリのSecretsに以下の3つの情報を登録してください。

    リポジトリの `Settings` > `Secrets and variables` > `Actions` に移動し、`New repository secret` ボタンから登録します。

    - `GMAIL_ADDRESS`
      - **説明**: 通知メールの送信元となるGmailアドレス。
      - **例**: `your.email@gmail.com`

    - `GMAIL_APP_PASSWORD`
      - **説明**: 送信元Gmailアカウントのアプリパスワード。通常のログインパスワードとは異なります。アプリパスワードはGoogleアカウントのセキュリティ設定から生成できます。
      - **参考**: [アプリ パスワードでログイン - Google アカウント ヘルプ](https://support.google.com/accounts/answer/185833)

    - `RECIPIENT_EMAIL`
      - **説明**: 通知メールを受け取る宛先のメールアドレス。
      - **例**: `notify.me@example.com`

3.  **GitHub Actionsの有効化**
    上記の設定が完了すると、`main.yml`ワークフローが有効になります。デフォルトでは、毎日3回（午前6時、正午12時、午後6時 JST）に自動実行されます。

## 手動での動作確認

すぐに動作を確認したい場合は、以下の手順で手動実行が可能です。

1.  リポジトリの **Actions** タブに移動します。
2.  左側のサイドバーから **Blog Update Checker** ワークフローを選択します。
3.  **Run workflow** というドロップダウンボタンが表示されるので、それをクリックしてワークフローを開始します。

## ローカル環境での実行（任意）

ローカルマシンでスクリプトを直接実行することも可能です。

1.  **依存ライブラリのインストール:**
    ```bash
    pip install -r requirements.txt
    ```

2.  **環境変数の設定:**
    ```bash
    export GMAIL_ADDRESS="your.email@gmail.com"
    export GMAIL_APP_PASSWORD="your_app_password"
    export RECIPIENT_EMAIL="notify.me@example.com"
    ```

3.  **スクリプトの実行:**
    ```bash
    python3 main.py
    ```

## 監視対象のUR物件

以下の9物件の空室状況を監視しています。

### 品川区（5物件）
- 大井六丁目
- 品川八潮パークタウン 潮路北第二ハイツ
- 品川八潮パークタウン 潮路南第一ハイツ
- 品川八潮パークタウン 潮路中央ハイツ
- コンフォール品川西大井

### 港区（1物件）
- デュプレ芝浦

### 目黒区（3物件）
- 恵比寿ビュータワー
- 中目黒ゲートタウンハイツ
- 中目黒アトラスタワー

## 通知の仕組み

### ブログ更新通知
- 新しい記事が投稿されると、記事のタイトル、URL、サムネイル画像を含むメールが送信されます

### UR空室通知
- 監視対象物件の空室数が増加すると、以下の情報を含むメールが送信されます：
  - 物件名と所在区
  - 空室数の変化（例: 0室 → 2室）
  - 物件詳細ページへのリンク
- **注意**: 空室が減少した場合（入居があった場合）は通知されません

## ファイル構成

- `main.py` - メインスクリプト（ブログチェック + UR空室チェック）
- `last_article.txt` - 最後にチェックしたブログ記事のURL
- `ur_properties.json` - UR物件の空室状態を記録するファイル
- `requirements.txt` - 必要なPythonパッケージ
- `.github/workflows/main.yml` - GitHub Actionsの設定
- `CLAUDE.md` - 開発者・AIアシスタント向けプロジェクト詳細ドキュメント
- `README.md` - このファイル（ユーザー向けドキュメント）

## 開発者向け情報

このプロジェクトを拡張・カスタマイズする場合は、`CLAUDE.md`を参照してください。以下の詳細情報が含まれています：

- コードの内部構造と関数の詳細
- 新しい監視対象を追加する方法
- トラブルシューティングガイド
- ローカル開発環境のセットアップ
- 設計原則とアーキテクチャの説明

AIアシスタント（Claude等）を使用してコードを修正・拡張する際も、CLAUDE.mdを読み込ませることでスムーズに作業できます。
